...
Chat:
```
Leia a spec completa em .dev/specs/H0.1-ONBOARDING-ATIVO.md, seções 3.1 a 3.3.
...
Chat:
```
Leia .dev/specs/H0.1-ONBOARDING-ATIVO.md seções 3.4 e 3.5 (schema e sub-opções do objetivo).
...
- **Validação campo a campo:** Após implementar, peça: "Compare o que foi implementado com .dev/excalidraw/02_ATIVO.md Passo X, campo a campo."

---

## 6. Checklist por História

Use este checklist antes de considerar uma história como "done":

- [ ] Spec lida e entendida pelo agente (fase chat)
- [ ] Plano revisado e aprovado (fase plan)
- [ ] Código implementado (fase agent)
- [ ] `npm run build` passa
- [ ] `npm run test` passa (se testes existem)
- [ ] Comparação campo a campo com Excalidraw
- [ ] Critérios de aceite da spec todos marcados
